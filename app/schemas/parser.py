...
class ImageParserResult(BaseModel):
    ...
    full_text: str = Field(
        ..., description="Toàn bộ text của ảnh nối bằng dấu xuống dòng"
    )
    results: List[ParserResultItem] = Field(
        default_factory=list, description="Danh sách các dòng chữ chi tiết"
    )


class DocumentParserBatchResponse(BaseModel):
    success: bool
    results: List[ImageParserResult] = Field(
        ..., description="Danh sách kết quả theo từng ảnh"
    )
    processing_time: float = Field(
        ..., description="Tổng thời gian xử lý toàn bộ pipeline (giây)"
    )
